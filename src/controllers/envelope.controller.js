      ...
      if (firstRecipient) {
        ...
        if (firstRecipient.role === 'SIGNER') {
          await emailService.sendSignatureRequestEmail({
            ...
            message: envelope.message || 'Merci de signer ce document.',
          ...
          await emailService.sendReviewRequestEmail({
            ...
            documentTitle: envelope.title,
            description: envelope.description || '',
            message: envelope.message || 'Merci de réviser ce document.',
          ...
          await emailService.sendApprovalRequestEmail({
            ...
            documentTitle: envelope.title,
            description: envelope.description || '',
            message: envelope.message || 'Merci d\'approuver ce document.',
...
exports.deleteEnvelope = async (req, res) => {
  try {
    ...
    // Suppression définitive de l'enveloppe
    ...
    const Signature = require('../models/Signature');
    await Signature.deleteMany({ envelopeId: id });

    console.log('✅ Enveloppe supprimée définitivement de la base de données');
...
exports.approveDocument = async (req, res) => {
  try {
    ...
    const { comment, metadata, fields: fieldData } = req.body;
    ...
    // Vérifier que le destinataire n'a pas déjà approuvé/révisé
    if (recipient.status === 'APPROVED' || recipient.status === 'REVIEWED') {
      const actionLabel = recipient.role === 'REVIEWER' ? 'révisé' : 'approuvé';
      return res.status(400).json({
        success: false,
        message: `Document déjà ${actionLabel}`,
      });
    }

    // Marquer comme approuvé ou révisé selon le rôle
    if (recipient.role === 'REVIEWER') {
      recipient.status = 'REVIEWED';
      recipient.action = 'REVIEW';
    } else if (recipient.role === 'APPROVER') {
    ...
    // Enregistrer les valeurs des champs (signatures, etc.)
    if (fieldData && Array.isArray(fieldData)) {
      const Field = require('../models/Field');

      for (const fieldSubmission of fieldData) {
        if (fieldSubmission.value && fieldSubmission.value.trim() !== '') {
          try {
            await Field.findByIdAndUpdate(
              fieldSubmission.fieldId,
              {
            ...
            console.log(`  ✅ Champ ${fieldSubmission.fieldId} rempli`);
          } catch (err) {
            console.error(`  ❌ Erreur enregistrement champ ${fieldSubmission.fieldId}:`, err);
          }
        }
      }
    }
    ...
    const reviewersValidated = reviewers.filter(r => r.status === 'REVIEWED' || r.status === 'APPROVED' || r.status === 'SIGNED').length;
    ...
    if (allReviewersValidated && allApproversValidated && allSignersValidated) {
      ...
      // Envoyer notification de completion à l'expéditeur (admin)
      const emailService = require('../services/emailService');
      const User = require('../models/User');
      const Client = require('../models/Client');
      ...
      try {
        ...
        if (sender && sender.email) {
          // Petit délai avant l'envoi pour éviter les problèmes de rate limiting
          await new Promise(resolve => setTimeout(resolve, 1000));
          ...
          await emailService.sendEnvelopeCompletedEmail({
            ...
            signedDocumentUrl: pdfInfo.fileUrl,
            clientSubdomain: client?.subdomain,
          });

          console.log('✅ Email de completion envoyé à l\'admin:', sender.email);
        }
      } catch (emailErr) {
        console.error('❌ Erreur envoi email de completion:', emailErr);
      ...
      if (envelope.workflow.type === 'SEQUENTIAL') {
        ...
        // Vérifier si tous les REVIEWER ont validé (status REVIEWED, APPROVED ou SIGNED)
        const allReviewers = envelope.recipients.filter(r => r.role === 'REVIEWER');
        const allReviewersValidated = allReviewers.every(r => r.status === 'REVIEWED' || r.status === 'APPROVED' || r.status === 'SIGNED');

        console.log(`📊 Reviewers validés: ${allReviewers.filter(r => r.status === 'REVIEWED' || r.status === 'APPROVED' || r.status === 'SIGNED').length}/${allReviewers.length}`);
...
exports.rejectDocument = async (req, res) => {
  try {
    ...
    // Enregistrer les métadonnées (seulement si elles existent)
    if (metadata) {
      if (!recipient.signatureMetadata) {
        recipient.signatureMetadata = {};
      }

      // Merger uniquement les champs définis
      if (metadata.userAgent) recipient.signatureMetadata.userAgent = metadata.userAgent;
      if (metadata.ipAddress) recipient.signatureMetadata.ipAddress = metadata.ipAddress;
      if (metadata.deviceType) recipient.signatureMetadata.deviceType = metadata.deviceType;
      if (metadata.geolocation) recipient.signatureMetadata.geolocation = metadata.geolocation;
      if (metadata.signatureMethod) recipient.signatureMetadata.signatureMethod = metadata.signatureMethod;
      if (metadata.signatureDuration) recipient.signatureMetadata.signatureDuration = metadata.signatureDuration;
    }
